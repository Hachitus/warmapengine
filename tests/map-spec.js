...
describe("basic map - without plugins", function() {
  ...
  describe("=> Test class construction", function() {
    ...
    describe("=> test sprite", function() {
      let spritesheet = _reset_spritesheets();
      let sprite = _reset_sprites(spritesheet);
...
/* ===== Test class inits ===== */
  ...
  describe("=> testing map (success)", function() {
    ...
    let layer = _createLayers(all.layerSize, all.layerObjects);
...
function _reset_spritesheets() {
  let spritesheetData = graphicData.terrain;
...
function _reset_stages() {
  let canvas = new Canvas( 980, 580 );
  let canvas2 = new Canvas( 980, 580 );
...
function _resetVariables() {
  let objects = [];
  let stages = _reset_stages();
  let layerObjects = _reset_layers();
  let spriteSheet = _reset_spritesheets();
  let sprites = _reset_sprites( spriteSheet );
  let map = _reset_map({ x: 100, y:100 });
